...
const ACCOUNTS_KEY = "google-calendar-accounts-v2";
...
// Handle callback — exchange code for tokens via our backend
export async function handleAuthCallback(): Promise<boolean> {
  const url = new URL(window.location.href);
  const code = url.searchParams.get("code");

  // Also handle legacy implicit flow tokens (backward compat)
  ...
  if (hash && hash.includes("access_token")) {
    ...
    if (token) {
      try {
        ...
        const data = await res.json();
        addAccount(data.email || "Unknown", token, "", 3600);
      } catch {
        addAccount("Account", token, "", 3600);
      }
